...
$sql= "select t.testID, t.testName, tr.resultID, tr.result, tr.startTime, tr.endTime, tr.emailed, tr.errorLog from tests t, testResults tr where t.testID=tr.testID";
$rs = $db ->Execute($sql);
...
if (!$rs) {
    ...
    while (!$rs->EOF) {
        ...
        if($errorLog=='NULL'||$errorLog='null'){
            echo '<td></td>';
        }
        else {
        echo '<td>'.$errorLog.'</td>';
